...
-- [1,2,3]
bufferToFile :: (CFConduitLike c1, CFConduitLike c2, Serialize x, MonadUnliftIO m, MonadIO m, MonadResource m, MonadThrow m)
                => Int -- ^ Size of the bounded queue in memory
...
stage :: (MonadUnliftIO m, MonadIO m) => TBQueue (Maybe i) -> Async x -> CConduit i Void m r -> m r
...
fsender :: (MonadThrow m, MonadIO m, MonadResource m, Serialize x) => BufferContext m x -> ConduitT () x m () -> m ()
...
fstage :: (MonadThrow m, MonadUnliftIO m, MonadIO m, MonadResource m) => ConduitT () i m () -> Async x -> CFConduit i Void m r -> m r
...
persistChan :: (MonadThrow m, MonadIO m, MonadResource m, Serialize o) => BufferContext m o -> STM (m ())
